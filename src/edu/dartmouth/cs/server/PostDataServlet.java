		...
		try {
			...
			for (int i=0; i < len; i++) {
				...
				entry.setDate(
					...
					entryJson.getInt(FIELD_MONTH) + 1,
					entryJson.getInt(FIELD_DATE)
				);
				entry.speechDoneCount = entryJson.getLong(FIELD_SPEECH_ATTEMPTS);
				entry.speechCorrectCount = entryJson.getLong(FIELD_SPEECH_CORRECT);
